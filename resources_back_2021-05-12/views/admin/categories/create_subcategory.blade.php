<div class="row">
	<div class="col-12">
		...
		<!-- /.card-header -->
		<!-- form start -->
		<form role="form" method="post" action="{{ route('admin.subcategory.store') }}" id="add_new">
			@csrf
			<div class="card-body">
				<div class="form-group">
	                ...
	                <select class="form-control" id="parent" name="parent" required>
	                    ...
	                    @foreach($categories as $category)
	                        <option {{ ($category->id == old('parent') ) ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->title }}</option>
	                    @endforeach
	                </select>
	            </div>
				<div class="form-group">
