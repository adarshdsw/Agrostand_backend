...
@section('button')
   <button type="button" class="btn btn-sm btn-outline-primary ml-3 pr-3 pl-3 add-unit">Add Unit</button>
@endsection
                        ...
                        <tbody id="unit_panel">
                              ...
                              @foreach($units as $unit)
                                 <tr>
                                    <td>{{ $counter }}</td>
                                    <td>{{ $unit->title }}</td>
                                    <td>{{ $unit->title_hindi }}</td>
                                    <td>{{ $unit->value }}</td>
                                    <td><?= ($unit->status == 0) ? "<span class='badge bg-danger'>Inactive</span>" : "<span class='badge bg-success'>Active</span>"; ?></td>
                                    <td>{{ $unit->created_at }}</td>
                                    <td>
                                       <a class="btn btn-xs btn-success ajax-edit" href="{{ route('admin.units.edit', $unit) }}" role="button" title="Edit"><i class="fas fa-pencil-alt"></i></a>
                                       <a class="btn btn-xs btn-danger ajax-delete" href="{{ route('admin.units.destroy', $unit) }}" role="button" title="Delete" data-menu_id="{{$unit->id}}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                 </tr>
                                    @php
                                       $counter++;
   ...
   var menu = (function () {
      ...
      var onReady = function () {
         ...
         $(document).on('click', '.add-unit', function () {
            var url = '{{ route("admin.units.create") }}';
            sbm.loadMultiPartForm(url, $(this), errorAjax, title);
         });
      }
