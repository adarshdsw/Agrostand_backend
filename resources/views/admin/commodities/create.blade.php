<div class="row">
	<div class="col-12">
		...
		<!-- /.card-header -->
		<!-- form start -->
		<form role="form" method="post" action="{{ route('admin.commodity.store') }}" id="add_new">
			@csrf
			<div class="card-body">
				<div class="form-group">
	                ...
	                <select class="form-control" id="subcategory_id" name="subcategory_id" required>
	                    ...
	                    @foreach($subcategories as $subcategory)
	                        <option {{ ($subcategory->id == old('subcategory') ) ? 'selected' : '' }} value="{{ $subcategory->id }}">{{ $subcategory->title }}</option>
	                    @endforeach
	                </select>
	            </div>
				<div class="form-group">
